...
- **POST / PATCH read/write-phase timeouts are never retried.** `BaseGandiClient._request` narrows `retry_on` to connection-phase failures (`ConnectError`, `ConnectTimeout`) only for non-GET/HEAD methods, preventing double-spend on purchase endpoints. A `ConnectTimeout` is itself a `TimeoutException` (not a `ConnectError`) but the connection never opened, so it is always retried and surfaced as `GandiConnectionError` — never the partial-write warning. `ReadTimeout`/`WriteTimeout` on a non-idempotent method surface as `GandiTimeoutError(method=...)`; `handle_client_error` uses `method` to warn the agent to check state before retrying.
